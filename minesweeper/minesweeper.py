...
class MinesweeperAI():
    ...
    def mark_new(self):
        ...
        # Looking for new sentences
        if len(self.knowledge) > 1:
            temp = []
            for sentence in self.knowledge:
                for other in self.knowledge:
                    if sentence.cells.issuperset(other.cells) and sentence != other:
                        new = sentence.cells - other.cells
                        count = sentence.count - other.count
                        temp.append(Sentence(cells=new, count=count))
    ...
    def add_knowledge(self, cell, count):
        ...
        # 5) add any new sentences to the AI's knowledge base 
        #    if they can be inferred from existing knowledge  
        temp_know = self.knowledge.copy()
        temp_safes = self.safes.copy()
        temp_mines = self.mines.copy()
        self.mark_new()
        while temp_know != self.knowledge or temp_safes != self.safes or temp_mines != self.mines:
            temp_know = self.knowledge
            temp_safes = self.safes.copy()
            temp_mines = self.mines.copy()
            self.mark_new()
